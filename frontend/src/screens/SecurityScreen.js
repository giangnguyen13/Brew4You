...
const SecurityScreen = () => {
  ...
  const _handleUpdateUserPassword = async (e) => {
    try {
        ...
        if(newPassword !== confirmPassword) {
            ...
            await api.put(END_POINTS.UPDATE_USER_PASSWORD, {password}, {...user_config}).then(response => {
  ...
  <Row>
    ...
    <Card.Text>Current password</Card.Text>
      <Form.Control type="password" required  value={formValues.currentPassword} onChange={_handleFormInputChange} name="currentPassword"/>
    </Col>
  </Row>
  <Row>
    <Col>
    <Card.Text>New password</Card.Text>
      <Form.Control type="password" required  value={formValues.newPassword} onChange={_handleFormInputChange} name="newPassword"/>
    </Col>
  </Row>
  <Row>
    <Col>
    <Card.Text>Confirm password</Card.Text>
      <Form.Control type="password" required  value={formValues.confirmPassword} onChange={_handleFormInputChange} name="confirmPassword"/>
    </Col>
  </Row>
  <Row >
    <Col style={{margin: '1rem' }}>
    <button type="submit" class="btn btn-success btn-md" onClick={_handleUpdateUserPassword}>Update</button>
    </Col>
  </Row>
</Form>
