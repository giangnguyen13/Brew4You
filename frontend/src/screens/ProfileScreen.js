...
const ProfileScreen = () => {
  ...
  const _handleUpdateProfileInfo = async () => {
    try {
      const {firstName, lastName, street, postalCode, city, province, country} = formValues;
      const address = {street, postalCode, city, province, country}
      const profile = {firstName, lastName, address}
      await api.put(END_POINTS.UPDATE_USER_PROFILE, {profile}, {...user_config}).then(response => {
  ...
  return (
    ...
    <Card.Img  variant="top" src="https://styles.redditmedia.com/t5_ma1hc/styles/profileIcon_snoo4c2ce00a-2f6b-4615-ba78-23ada548a710-headshot.png" />
  ...
  <Row>
    ...
    <Card.Text> First Name</Card.Text>
      <Form.Control placeholder={formValues.firstName} value={formValues.firstName} onChange={_handleFormInputChange} name="firstName"/>
    </Col>
    <Col>
      <Card.Text> Last Name</Card.Text>
      <Form.Control placeholder={formValues.lastName} value={formValues.lastName} onChange={_handleFormInputChange} name="lastName"/>
    </Col>
  ...
  <Col>
      ...
      <Form.Control placeholder={formValues.address.street} value={formValues.street} onChange={_handleFormInputChange} name="street"/>
    </Col>
  </Row>
  <Row>
  <Col>
      <Card.Text> City </Card.Text>
      <Form.Control placeholder={formValues.address.city} value={formValues.city} onChange={_handleFormInputChange} name="city"/>
    </Col>
    <Col>
      <Card.Text> Postal Code </Card.Text>
      <Form.Control placeholder={formValues.address.postalCode} value={formValues.postalCode} onChange={_handleFormInputChange} name="postalCode"/>
    </Col>
  </Row>
  <Row>
  <Col>
      <Card.Text> Province </Card.Text>
      <Form.Control placeholder={formValues.address.province} value={formValues.province} onChange={_handleFormInputChange} name="province" />
    </Col>
    <Col>
      <Card.Text> Country </Card.Text>
      <Form.Control placeholder={formValues.address.country} value={formValues.country} onChange={_handleFormInputChange} name="country"/>
    </Col>
  </Row>
  <Row>
    <Col style={{margin: '1rem'}}>
    <button type="button" class="btn btn-success btn-md" onClick={_handleUpdateProfileInfo}>Save</button>
